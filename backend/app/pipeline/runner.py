...
def run_pipeline_sync(db: Session, job: Job) -> Job:
    ...
    # Terminal-state rule: any failed stage makes the job failed; only a fully
    # successful chain marks the job successful. Error details are retained.
    job.status = "success" if success else "failed"
    job.metrics = ctx.metrics if success else (ctx.metrics or None)
    job.error_message = None if success else (ctx.error or "流水线执行失败")
